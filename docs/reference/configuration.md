...
| `box.maxHours` | int | `0` (= `12`) | A box's outside limit: however the run is going, the box stops it and destroys itself this many hours after it booted. The backstop for the run that hangs rather than ends — a box nothing ever finishes on is a box nothing ever destroys. It never destroys a machine holding commits that exist nowhere else; it retries hourly instead. `--max-hours` sets it for one run. |
