...
| `box.type` | string | `""` (= `cpx32`) | Hetzner server type for a box, e.g. `cpx32` (4 vCPU, 8 GB). Set it with `chief box config`, which shows each machine's price and what five hours of it would cost; `--type` overrides it for one run. |
