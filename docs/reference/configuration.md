...
| `box.type` | string | `""` (= `cx33`) | Hetzner server type for a box, e.g. `cx33` (4 vCPU, 8 GB). Set it with `chief box config`, which shows each machine's price and what five hours of it would cost; `--type` overrides it for one run. |
| `box.image` | string | `""` (= `ubuntu-26.04`) | Hetzner image a box is built from. Rarely worth setting: the default is the current Ubuntu LTS, and the cloud-config that provisions the box assumes the package names that LTS ships. |
| `box.packages` | list | `[]` | Apt packages this project needs on the box, on top of the base (PHP, Node, PostgreSQL, Redis, Composer, the GitHub CLI, Claude Code). For the dependency that is none of those. `--package` replaces the list for one run. |
