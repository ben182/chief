...
worktree:
  ...
  setupTimeoutSeconds: 0  # kill a setup that runs longer than N seconds; 0 = wait forever
onComplete:
  push: true
  createPR: true
  ...
  notify: true       # desktop notification when the run stops, finished or not
loop:
...
| `worktree.dir` | string | `""` | Path template saying where a PRD's worktree goes. Empty (the default) means `.chief/worktrees/{prd}`. Placeholders: `{prd}` (the PRD name), `{repo}` (the basename of the main checkout) and `{branch}` (the worktree's branch, with `/` replaced by `-`). A relative template is resolved against the main checkout, so `../{repo}-worktrees/{prd}` puts worktrees beside the project — useful when a bundler, test runner or editor index trips over checkouts nested inside the checkout. A template resolving to the main checkout itself or to its bare parent directory is **rejected before anything is created**, with a message naming this key. Chief finds worktrees at the configured location again through `git worktree list`, so orphan detection and cleanup follow the template. A location **inside** the repository is added to the main checkout's `.gitignore` before the worktree is created — the directory holding the worktrees (`.chief/worktrees/` for the default), not each worktree — unless `git check-ignore` says something already covers it. A location outside the repository, or one directly in its root, is left alone. |
...
| `onComplete.createPR` | bool | `false` | Automatically create a pull request when a PRD completes (requires `gh` CLI). Only runs after a successful push, so a run with no commits creates no PR. The PR targets the branch the run's branch was cut from, and an already-open PR for the branch is reported instead of a second one being opened — see [Pull request target](#pull-request-target). A newly opened PR is assigned to the account `gh` is authenticated as, so an unattended run — a [box](/reference/cli#chief-box) finishing overnight — lands in your "Assigned to you" list rather than among the branches. That is where it is found, not a notification: the run authenticates as the same account, and GitHub does not report your own actions back to you. Assigning is best-effort and never costs the PR. |
...
| `onComplete.notify` | bool | `true` | Send a desktop notification when a run stops — finished, capped by `maxIterations`, or dead on an error it could not retry past (macOS `osascript`, Linux `notify-send`). |
...
| `box.location` | string | `""` (= `fsn1`) | Hetzner location a [`chief box`](/reference/cli#chief-box) run is created in, e.g. `fsn1` for Falkenstein. The box holds the project's source and its `.env` for the length of a run, so this decides which country that happens in. Set it with `chief box config`, which lists what Hetzner actually offers today; `--location` overrides it for one run. |
| `box.type` | string | `""` (= the cheapest the location sells) | Hetzner server type for a box, e.g. `cpx32` (4 vCPU, 8 GB). Left empty, every box is created on the cheapest machine its location offers that is not being retired, asked of Hetzner at the moment of creation. Set it with `chief box config`, which shows each machine's price and what five hours of it would cost; `--type` overrides it for one run. |
| `box.image` | string | `""` (= `ubuntu-24.04`) | Hetzner image a box is built from. Rarely worth setting: PHP comes from a PPA that publishes for this release, and a newer one may have no PHP at all. |
| `box.php` | string | `""` (= detected) | PHP series the box installs, e.g. `8.3`. Left empty, chief uses the version Herd isolates this site to, else the `php` on this machine, else the lowest `composer.json` accepts. `--php` overrides it for one run. |
| `box.node` | string | `""` (= detected) | Node major the box installs, e.g. `22`. Left empty, chief uses the `node` on this machine, else `.nvmrc`, else the lowest `package.json` accepts. `--node` overrides it for one run. |
| `box.packages` | list | `[]` | Apt packages this project needs on the box, on top of [what chief read from the project](/reference/cli#what-the-box-is-built-with). For the dependency no manifest declares. `--package` adds to the list for one run. |
| `box.files` | list | `[".env"]` | Paths, relative to the project, that git does not carry but the run needs. `--file` replaces the list for one run. |
| `box.keep` | bool | `false` | Leave a box standing after its run ends. By default a box [destroys itself](/concepts/the-box#the-box-switches-itself-off) once its work is on origin — the alternative is a machine billing until somebody notices, and the person who would notice is asleep, which is the whole reason the run is on a box. Set this for a project whose boxes you want to inspect afterwards, and remember that `chief box down` is then the only thing that stops the bill. `--keep` sets it for one run. |
| `box.maxHours` | int | `0` (= `12`) | When a box starts checking on its run, in hours after boot: from then on a run that has not committed in six hours is stopped and the box destroys itself, while one that is still committing goes on, up to four times this after it started. The backstop for the run that hangs rather than ends — a box nothing ever finishes on is a box nothing ever destroys. It never destroys a machine holding commits that exist nowhere else; it retries hourly instead. `--max-hours` sets it for one run. |
...
Both commands are watched while they run and recorded in full. The spinner shows the last few lines of the running setup — mixed stdout and stderr, in the order the script wrote them — so a multi-minute setup is something to watch rather than a frozen screen.

The complete output lands in the PRD directory, one file per run:
...
Most of these reach a run that is already in flight, so you can react to what you're watching instead of stopping the run first:

| Setting | Takes effect |
|---------|--------------|
| `review.*`, `consolidate.*` | The next review or consolidation pass. A review already under way finishes on the settings it started with, so switching the model mid-story doesn't restart anything. |
| `loop.watchdogTimeoutSeconds` | The next agent invocation. The one currently running keeps the timeout it started with. |
